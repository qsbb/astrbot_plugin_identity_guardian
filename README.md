...
| 模块 | 能力 |
| --- | --- |
| 身份识别 | bot / 发送者 / 目标在群中的 role（owner / admin / member）查询与缓存；缓存键为「平台实例 + 群 + 用户」，多平台 / 多 Bot 下相同群号与 QQ 号各自独立取角色，互不串用 |
...
| 申请推送 | 有人申请进群且进入人工待审时，自动推送到该群配置的推送群（留空回退推送到申请所属群本身）；推送与审批结果回复都由 AI 按人设自由措辞（只给事实不写死模板；natural 整段生成，formatted 附 LLM 一句话看法、失败回退自动审核结论行，结果回复 LLM 失败回退固定文案），并引导管理员引用推送消息回复「同意/不同意」直接审批（该引用回复被消费后不再进入主对话），或到入群审核管理页处理；推送与旧版待审通知按群去重，同一群只收一条 |
...
| 紧急停止 | `/idg stop` 一键停用所有管理工具，仅保留身份注入；停止期间定时身份刷新与待审清理任务暂停，`/idg resume` 恢复后继续运行 |
| 知识库联动 | 入群审核可查询 active_learner 知识库作为判定证据 |

### 主动加群结果

`join_group` 会先读取 Bot 当前已加入的群列表。若目标群已在列表中，工具返回
`already_member` 并跳过重复申请；若未加入才调用适配器明确提供的加群扩展。适配器
拒绝、超时或不支持时，工具返回受限的状态码和平台原因摘要（不包含调用参数），
由主链路结合人格生成最终回复。
...
2. 本插件零第三方依赖（全部使用 AstrBot 内置能力），无需安装额外依赖。
...
重复下发的同一申请事件具有幂等性：OneBot 可能重复推送同一请求事件，序按平台请求
ID 归并到同一条记录——已终态（`approved` / `rejected` / `expired`）的记录直接返回
已处理，不重跑自动审核、不再发通知或推送；仍可人工处理的记录只重试此前失败的
通知/推送投递，不会再次调用平台批准/拒绝接口，同一申请不会产出重复消息或重复
平台动作。
...
若该群关闭「显示答案」，为避免自然语言 LLM 复述隐藏答案，推送会确定性回退为不含答案的格式化模板。
有申请进入
人工待审时会自动推送到这些群；推送群留空时回退推送到申请所属群本身。同一申请对同一
推送群只推一次；紧急停止或熔断期间不推送。推送与旧版待审通知按群去重——推送覆盖的
群不再重复发旧模板通知，同一群只收一条。Page 群配置表只保留摘要列，以上细分设置
均在点击群名打开的悬浮窗中编辑保存。
...
引用回复被消费后即调 `stop_event`，不会再进入主对话 LLM 造成二次回复；
bot 自己引用推送消息的回显会被直接忽略。
...
入群审核 Page 顶部的「全局设置」卡片可直接修改两个全局配置项，与 AstrBot 插件
配置页互通、保存后立即生效（原子写回 + 运行态 Config 重建，失败不落盘）：

- **审核模型**（`audit_llm_provider`）：下拉选择当前实例可用的 LLM provider，
  默认项为「默认（主对话 LLM）」（留空回退主对话模型）；模型列表获取失败时
  仅显示当前生效值并提示到插件配置页修改；
- **知联动开关**（`enable_active_learner_recall`）：预设未命中时查询知知识库
  辅助判定。
...
（LLM 生成 / 自动审核结论 / 无）。下方还有**结果回复预览**：「若同意/若拒绝」两行，
与生产审批结果回复共用同一渲染链路（同一 prompt、目标群人格、push LLM），LLM
不可用或失败时显示固定回退文案并标注「模板」。预览只生成不发送；注意这是独立生成调用，
...
除 `/idg help` 外，状态、停止、恢复、刷新、熔断重置和确认审批均要求 AstrBot 管理员权限。批准与拒绝操作都只能在创建确认的原群执行，且都受 API 层安全护栏约束——紧急停止或熔断期间审批会被拦截；批准会按审批时的实时身份重新运行策略，校验失败时确认记录保持待审。
